use crate::ir::{
    IrBinOp, IrFunc, IrGlobalInit, IrInst, IrModule, IrUnaryOp, LabelId, LocalId, TempId,
};
...
pub struct CodeGenerator {
    ir: IrModule,
    lines: Vec<String>,
    optimizable_insts: Vec<OptimizableInst>,
...
impl CodeGenerator {
    pub fn new(ir: IrModule, out: Box<dyn std::io::Write>) -> Self {
        Self {
            ir,
            lines: Vec::new(),
            optimizable_insts: Vec::new(),
            out,
            stack_frame: StackFrame::new(),
        }
    }

    pub fn lower(mut self) -> std::io::Result<Vec<String>> {
        self.lower_funcs();
        self.lower_strings();
        self.lower_globals();
    ...
    fn optimize(&mut self) {
        ...
        while i < self.lines.len() {
            if i + 1 < self.lines.len() {
                match (&self.optimizable_insts[i], &self.optimizable_insts[i + 1]) {
                    (
                        OptimizableInst::Load {
                            reg: reg1,
                            addr: addr1,
                        },
                        OptimizableInst::Store {
                            reg: reg2,
                            addr: addr2,
                        },
    ...
    fn emit_label(&mut self, label: &str) {
        ...
        self.optimizable_insts.push(OptimizableInst::Other);
    }

    fn emit_label_id(&mut self, func_label: &str, label: LabelId) {
        self.lines.push(format!(".label {}L{}", func_label, label));
        self.optimizable_insts.push(OptimizableInst::Other);
    }

    fn emit_inst(&mut self, inst: &str) {
        self.lines.push(format!("\t{}", inst));
        let parts: Vec<&str> = inst.split_whitespace().collect();
        if parts.len() == 3 && (parts[0] == "load" || parts[0] == "loadb") {
            let reg = parts[1].trim_end_matches(',');
            let addr = parts[2].trim_matches(&['[', ']']);
            self.optimizable_insts.push(OptimizableInst::Load {
                reg: reg.to_string(),
                addr: addr.to_string(),
            })
        } else if parts.len() == 3 && (parts[0] == "store" || parts[0] == "storeb") {
            let addr = parts[1].trim_end_matches(',').trim_matches(&['[', ']']);
            let reg = parts[2];
            self.optimizable_insts.push(OptimizableInst::Store {
                reg: reg.to_string(),
                addr: addr.to_string(),
            });
        } else {
    ...
    fn collect_temps(func: &IrFunc) -> HashSet<TempId> {
        ...
        for inst in &func.body {
            match inst {
                ...
                IrInst::AddrOfLocal { dst, .. } => {
                    temps.insert(*dst);
                }
                IrInst::LoadLocal { dst, .. } => {
                    temps.insert(*dst);
                }
                IrInst::StoreLocal { src, .. } => {
                    temps.insert(*src);
                }
                IrInst::LoadGlobal { dst, .. } => {
                    temps.insert(*dst);
                }
                IrInst::StoreGlobal { src, .. } => {
                    temps.insert(*src);
                }
                IrInst::AddrOfGlobal { dst, .. } => {
                    temps.insert(*dst);
                }
                IrInst::AddrOfFunc { dst, .. } => {
                    temps.insert(*dst);
                }
        ...
        for (global_id, global) in self.ir.globals.iter() {
            ...
            match &global.init {
                IrGlobalInit::Int(value) => {
                    self.lines
                        .push(format!(".label {} .word {}", global_label, value));
                }
                IrGlobalInit::Char(value) => {
                    self.lines
                        .push(format!(".label {} .data {}", global_label, *value as u32));
                }
                IrGlobalInit::StringPtr(string_id) => {
                    let string_label = self.ir.strings.label_for(*string_id);
                    self.lines
                        .push(format!(".label {} .word {}", global_label, string_label));
                }
                IrGlobalInit::Zero => {
                    self.lines.push(format!(".label {} .word 0", global_label));
                }
            }
            self.optimizable_insts.push(OptimizableInst::Other);
        }
    ...
    fn fn_prologue(&mut self, func: &IrFunc) -> HashMap<LocalId, i32> {
    ...
    fn lower_start(&mut self, func: &IrFunc) {
        let func_label = self.ir.functions.label_for(func.entry_label);
    ...
    fn lower_func(&mut self, func: &IrFunc) {
        let func_label = self.ir.functions.label_for(func.entry_label);
